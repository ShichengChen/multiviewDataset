...
class HandPoseLegitimizeLayer(nn.Module):
    def __init__(self,fingerPlanarize=True,fingerBoneLengthLegitimize=True,flexionLegitimize=True,abductionLegitimize=True,
                 planeRotationLegitimize=True,debug=False,r=8,relaxplane=False):
        ...
        self.debug=debug
        self.r=r
        print("fingerPlanarize,flexionLegitimize,abductionLegitimize,planeRotationLegitimize,r,relaxplane",
    ...
    def FingerPlanarize(joints: torch.Tensor,relaxplane=False) -> torch.Tensor:
        ...
        for idxfinger,finger in enumerate(jidx):
            mcppip = unit_vector(joints[:, finger[1]] - joints[:, finger[0]])
            pipdip = unit_vector(joints[:, finger[2]] - joints[:, finger[1]])
            diptip = unit_vector(joints[:, finger[3]] - joints[:, finger[2]])
            maskmcppipdipline = (torch.abs(torch.sum(mcppip * pipdip, dim=1)) > 0.96)
            maskpipdiptipline = (torch.abs(torch.sum(diptip * pipdip, dim=1)) > 0.96)
            maskline = maskmcppipdipline & maskpipdiptipline
            vh, vd = getPlaneFrom4Points(joints[:, finger].clone())
            for idx in range(4):
                #norms[:,finger[idx]]=vh.clone().reshape(N,3)
                if(relaxplane):
                    if idx<=2:
                        _, njoints[:, finger[idx]] = projectPoint2Plane(joints[:, finger[idx]], vh, vd,relaxValue=0.001)
                    elif idx==3:
                        _, njoints[:, finger[idx]] = projectPoint2Plane(joints[:, finger[idx]], vh, vd, relaxValue=0.002)
                else:
                    _, njoints[:, finger[idx]] = projectPoint2Plane(joints[:, finger[idx]], vh, vd)
                njoints[:,finger[idx]][maskline]=joints[:, finger[idx]][maskline].clone()
        return njoints
    ...
    def PlaneRotationLegitimize(self,joints:torch.Tensor)->torch.Tensor:
        ...
        for idx, finger in enumerate(jidx):
            for yaxis in range(1, 2):
                ...
                mcppip,pipdip,maskline,overextensionmask,stdfingerPlaneDir,palmNorm,calculatedFingerDir=\
    ...
    def getoverextensionMask(self,joints: torch.Tensor,mcpidx):
        fingeridx={1:[1,2,3,17],4:[4,5,6,18],10:[10,11,12,19],7:[7,8,9,20],}
        ...
        diptip = unit_vector(joints[:, fi[3]] - joints[:, fi[2]])
        maskmcppipdipline = (torch.abs(torch.sum(mcppip * pipdip, dim=1)) > 0.95)
        #print('maskval',torch.abs(torch.sum(mcppip * pipdip, dim=1)))
        #maskpipdiptipline = (torch.abs(torch.sum(diptip * pipdip, dim=1)) > 0.95)
        #maskline=maskmcppipdipline&maskpipdiptipline
...
if __name__ == "__main__":
    ...
    # plt.show()
    from cscPy.mano.network.manolayer import MANO_SMPL
    hpl=HandPoseLegitimizeLayer(debug=True,r=1000)
...
'''
    def AbductionLegitimizeOri(self, joints: torch.Tensor) -> torch.Tensor:
        device = joints.device
        N = joints.shape[0]
        ...
        r = 18
        # r = 180
        angleP = torch.tensor([np.pi / r + 0.1890, np.pi / r + 0.1331, np.pi / r - 0.1491, np.pi / r + 0.0347],
                              device=joints.device, dtype=joints.dtype)
        angleN = torch.tensor([np.pi / r - 0.1890, np.pi / r - 0.1331, np.pi / r + 0.1491, np.pi / r - 0.0347],
                              device=joints.device, dtype=joints.dtype)
        njoints = joints.clone()
        childern = [[2, 3, 17], [5, 6, 18], [11, 12, 19], [8, 9, 20], [14, 15, 16]]
        for i in range(4):
            ...
            mcpprojpip = unit_vector(projpip - mcp).reshape(N, 3)
            mcppip = unit_vector(pip - mcp).reshape(N, 3)
            sign = torch.sum(torch.cross(wristmcp, mcpprojpip, dim=1) * palmNorm, dim=1)
            maskP = (sign >= 0)
            maskN = (sign < 0)

            angle = torch.acos(torch.clamp(torch.sum(wristmcp * mcpprojpip, dim=1), -1 + epsilon, 1 - epsilon)).reshape(
                -1)
            # stdfingerPlaneDir = getFingerStdDir(joints, fingerStdDiridx[i]).reshape(N, 3)
            # calculatedFingerDir = unit_vector(torch.cross(wristmcp, mcppip, dim=1)).reshape(N, 3)
            # overextensionmask = torch.sum(calculatedFingerDir * stdfingerPlaneDir, dim=1).reshape(N) < 0
            overflexionmask = torch.acos(
                torch.clamp(torch.sum(wristmcp * mcppip, dim=1), -1 + epsilon, 1 - epsilon)).reshape(
                -1) > 3.14 / 2
            ...
            difangle = 0
            if (torch.sum(maskP)):
                difangle = torch.max(angle[maskP] - angleP[i], torch.zeros_like(angle[maskP])) * flexRatio[maskP]
                rot[maskP] = rotation_matrix(axis=palmNorm[maskP], theta=-difangle)
            if (torch.sum(maskN)):
                difangle = torch.max(angle[maskN] - angleN[i], torch.zeros_like(angle[maskN])) * flexRatio[maskN]
                rot[maskN] = rotation_matrix(axis=palmNorm[maskN], theta=difangle)
            ...
            if (self.debug and torch.sum(difangle) > 1e-3 and N == 1):
                pass
                print(i, rot, angle / 3.14 * 180, sign)
                print('abdcution child', childern[i])
                visMeshfromJoints(njoints)
        return njoints
